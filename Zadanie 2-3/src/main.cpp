...
void setup() //Konfiguracja trybu pracy wyjść
{
  ...
  pinMode(PED_BUTTON, OUTPUT);
  //USTALENIE POCZĄTKOWEJ WARTOŚCI NA WYJŚCIACH ŚWIATEŁ
...
void loop() //Głowna pętla programu
{
...
case RED:
    ...
    if((millis() - changeTime) > (STATE_3_TIME)){
      for(uint8_t i = 0; i < 3; i++){  // Trzykrotny błysk dla światła dla pieszych
        digitalWrite(PED_GREEN, LOW);
        delay(500);
        digitalWrite(PED_GREEN, HIGH);
        delay(500);
